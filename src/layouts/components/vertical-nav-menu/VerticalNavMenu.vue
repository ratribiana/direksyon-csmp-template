      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <div class="header-sidebar flex items-end justify-between" slot="header">
          ...
          <transition name="fade2">
            <img :src="logoUrl" :style="{width: reduce ? '35px' : '150px'}">
          </transition>
          <router-link tag="div" class="vx-logo cursor-pointer flex items-center" to="/">
            <logo class="w-10 mr-4 fill-current text-primary" v-if="false" />
            <span class="vx-logo-text text-primary" v-show="isMouseEnter || !reduce" v-if="title && false">{{ title }}</span>
          </router-link>
          <!-- /Logo -->
          ...
          <div>
            <!-- Close Button -->
            <template v-if="showCloseButton && false">
...
export default {
  ...
  methods: {
    ...
    toggleReduce (val) {
      ...
      if (this.reduce) {
        this.logoUrl = '/logo-small-blue.png'
      } else {
